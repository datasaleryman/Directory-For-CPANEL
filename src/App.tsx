...
export default function App() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-slate-50 flex flex-col md:flex-row font-sans antialiased print:h-auto print:overflow-visible">
      ...
      {/* Primary Sidebar Control Panel - Minimalist Architectural Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 no-print border-r border-slate-800/90 transition-transform duration-200 ease-in-out
      ...
      `}>
        <div className="p-5 border-b border-slate-800/90 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img 
              ...
              alt="Logo" 
              className="w-8 h-8 rounded-lg object-contain bg-white border border-slate-700/60 shadow-xs" 
              referrerPolicy="no-referrer"
            />
            <div className="min-w-0">
              <h1 className="font-semibold text-white text-sm tracking-tight truncate">
            ...
            .map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-medium transition-colors cursor-pointer group focus:outline-none ${
                    isActive
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {unreadInboxCount > 0 && (
                      <span className="px-1.5 py-0.2 bg-rose-500 text-white font-bold text-[10px] rounded-md tabular-nums">
                        {unreadInboxCount}
                      </span>
                    )}
                    {isActive && (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <Database className={`w-4 h-4 shrink-0 transition-colors ${
                      isSubTabActive ? 'text-emerald-400' : 'text-slate-400 group-hover:text-slate-200'
                    }`} />
                    <span className="truncate">Data Entry</span>
                  </div>
                  <ChevronDown className={`w-3.5 h-3.5 text-slate-500 group-hover:text-slate-300 transition-transform duration-150 ${
                    isDataEntryOpen ? 'rotate-180' : ''
                  }`} />
                </button>

                {isDataEntryOpen && (
                  <div className="pl-4 ml-3 border-l border-slate-800 space-y-1 mt-1">
                    {visibleDataEntryItems.map((item) => {
                      ...
                      return (
                        <button
                          key={item.id}
                          onClick={() => handleTabChange(item.id)}
                          className={`w-full flex items-center justify-between px-2.5 py-2 rounded-md text-xs font-medium transition-colors cursor-pointer group focus:outline-none ${
                            isActive
        ...
        <div className="p-4 border-t border-slate-800/80 bg-slate-950/50 text-center text-[11px] text-slate-500 font-medium shrink-0">
      ...
      <main className="flex-1 min-w-0 overflow-y-auto bg-slate-50/70 print:overflow-visible print:h-auto print:bg-white">
        {/* Header - Minimalist Top Bar */}
        <header className="bg-white border-b border-slate-200/80 py-3.5 px-4 sm:px-6 md:px-8 flex items-center justify-between no-print sticky top-0 z-40 text-slate-900 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-colors cursor-pointer focus:outline-none"
                  ...
                  <>
                    ...
                    >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3.5 py-2 text-slate-700 hover:bg-slate-50 font-medium text-xs text-left transition-colors cursor-pointer border-b border-slate-100"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3.5 py-2 text-rose-600 hover:bg-rose-50 font-medium text-xs text-left transition-colors cursor-pointer"
                      >
