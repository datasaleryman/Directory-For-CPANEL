...
export default function App() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-slate-50 flex flex-col md:flex-row font-sans antialiased print:h-auto print:overflow-visible">
      ...
      {/* Primary Sidebar Control Panel - Neumorphic Dark Green / Black Theme */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-[#051f15] text-emerald-100 flex flex-col shrink-0 no-print border-r border-[#0a3826] shadow-[4px_0_20px_rgba(2,44,34,0.35)] transition-transform duration-200 ease-in-out
        md:static md:translate-x-0
        ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-5 border-b border-[#0a3826] bg-[#02170f] flex items-center justify-between shadow-xs">
          <div className="flex items-center gap-3">
            <img 
              src={siteSettings.logoDataUrl || DEFAULT_SITE_LOGO} 
              alt="Logo" 
              className="w-8 h-8 rounded-xl object-contain bg-white border border-emerald-500/30 shadow-[2px_2px_6px_rgba(0,0,0,0.3)] p-0.5" 
              referrerPolicy="no-referrer"
            />
            <div className="min-w-0">
              <h1 className="font-bold text-white text-sm tracking-tight truncate font-display">
                {siteSettings.faviconTitle || 'Saint Francis Clinic'}
              </h1>
              <span className="text-[11px] text-emerald-400/90 font-medium block truncate">
                Clinic Portal
              </span>
            </div>
          </div>
          ...
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="md:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-emerald-400 hover:text-white rounded-xl hover:bg-emerald-900/50 transition-colors cursor-pointer"
            title="Close menu"
        ...
        <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto">
          {([
            ...
            .map((item) => {
              ...
              const unreadInboxCount = item.id === 'inbox' ? messagesList.filter(m => !seenMessageIds.includes(m.id)).length : 0;

              return (
                <button
                  key={item.id}
                  onClick={() => handleTabChange(item.id)}
                  className={`w-full flex items-center justify-between px-3 py-2.5 min-h-[44px] rounded-xl text-xs font-medium transition-all cursor-pointer group focus:outline-none ${
                    isActive
                      ? 'bg-gradient-to-r from-[#064e3b] to-[#047857] text-white font-bold shadow-[3px_3px_8px_rgba(2,44,34,0.6),-1px_-1px_4px_rgba(52,211,153,0.2)] border border-emerald-400/30'
                      : 'text-emerald-200/80 hover:text-white hover:bg-emerald-900/30 hover:shadow-[inset_1px_1px_3px_rgba(0,0,0,0.2)]'
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <Icon className={`w-4 h-4 shrink-0 transition-colors ${
                      isActive ? 'text-emerald-300' : 'text-emerald-400/80 group-hover:text-emerald-200'
                    }`} />
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {unreadInboxCount > 0 && (
                      <span className="px-1.5 py-0.2 bg-rose-500 text-white font-bold text-[10px] rounded-md tabular-nums shadow-xs">
                        {unreadInboxCount}
                      </span>
                    )}
                    {isActive && (
            ...
            return (
              <div className="pt-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 min-h-[44px] rounded-xl text-xs font-medium transition-all cursor-pointer group focus:outline-none ${
                    isSubTabActive 
                      ? 'text-white bg-gradient-to-r from-[#064e3b] to-[#047857] font-bold shadow-[3px_3px_8px_rgba(2,44,34,0.6)] border border-emerald-400/30' 
                      : 'text-emerald-200/80 hover:text-white hover:bg-emerald-900/30'
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <Database className={`w-4 h-4 shrink-0 transition-colors ${
                      isSubTabActive ? 'text-emerald-300' : 'text-emerald-400/80 group-hover:text-emerald-200'
                    }`} />
                    <span className="truncate">Data Entry</span>
                  </div>
                  <ChevronDown className={`w-3.5 h-3.5 text-emerald-400/70 group-hover:text-emerald-200 transition-transform duration-150 ${
                    isDataEntryOpen ? 'rotate-180' : ''
                  }`} />
                </button>

                {isDataEntryOpen && (
                  <div className="pl-3 ml-3 border-l border-emerald-800/60 space-y-1 mt-1">
                    {visibleDataEntryItems.map((item) => {
                      ...
                      return (
                        <button
                          key={item.id}
                          onClick={() => handleTabChange(item.id)}
                          className={`w-full flex items-center justify-between px-2.5 py-2.5 min-h-[40px] rounded-lg text-xs font-medium transition-all cursor-pointer group focus:outline-none ${
                            isActive
                              ? 'text-white bg-[#064e3b] font-bold border border-emerald-400/40 shadow-[2px_2px_6px_rgba(2,44,34,0.5)]'
                              : 'text-emerald-300/70 hover:text-white hover:bg-emerald-900/25'
                          }`}
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <Icon className={`w-3.5 h-3.5 shrink-0 ${
                              isActive ? 'text-emerald-300' : 'text-emerald-500/80 group-hover:text-emerald-300'
                            }`} />
                            <span className="truncate">{item.label}</span>
                          </div>
                          {isActive && (
        ...
        <div className="p-4 border-t border-[#0a3826] bg-[#02170f] text-center text-[11px] text-emerald-400/60 font-medium shrink-0">
      ...
      <main className="flex-1 min-w-0 overflow-y-auto bg-[#f1f5f2] print:overflow-visible print:h-auto print:bg-white">
        {/* Header - Neumorphic Top Bar with <=15% Mobile Sticky Cap */}
        <header className="bg-white/95 backdrop-blur-md border-b border-emerald-900/10 py-2.5 sm:py-3.5 px-3.5 sm:px-6 md:px-8 flex items-center justify-between no-print sticky top-0 z-40 text-slate-900 shadow-[0_4px_16px_rgba(6,78,59,0.04)]">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="md:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-emerald-800 hover:text-emerald-950 hover:bg-emerald-50 rounded-xl transition-colors cursor-pointer shrink-0"
            ...
            <img 
              src={siteSettings.logoDataUrl || DEFAULT_SITE_LOGO} 
              alt="Logo" 
              className="md:hidden w-7 h-7 rounded-md object-contain bg-white border border-slate-200 shrink-0" 
              referrerPolicy="no-referrer"
            />
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-400 mt-0.5 truncate hidden sm:block">
                Workspace · {new Date().toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 min-h-[44px] px-2 sm:px-2.5 py-1.5 rounded-xl hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-colors cursor-pointer focus:outline-none"
              ...
              >
                ...
                <div className="hidden sm:block text-left">
                  ...
                  <p className="text-[10px] text-slate-400 leading-tight capitalize">{adminUser.role}</p>
                </div>
                ...
                {isProfileDropdownOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 mt-1.5 w-48 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-50 text-slate-800"
                    >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3.5 py-2.5 min-h-[40px] text-slate-700 hover:bg-slate-50 font-medium text-xs text-left transition-colors cursor-pointer border-b border-slate-100"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3.5 py-2.5 min-h-[40px] text-rose-600 hover:bg-rose-50 font-medium text-xs text-left transition-colors cursor-pointer"
                      >
        ...
        {/* Tab Router Panels */}
        <div className="p-3.5 sm:p-6 md:p-8 max-w-full overflow-x-hidden">
